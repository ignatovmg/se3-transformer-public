...
def calc_rmsd(y_pred, y_true):
    return torch.sqrt(torch.power(y_pred - y_true, 2).sum(1).mean(0))


def refine(rec_pdb, lig_pdb, lig_mol, device='cuda:0', nsteps=3, crys_mol=None):
    model = load_model(device)
    rec_ag = prody.parsePDB(rec_pdb)
    lig_ag = prody.parsePDB(lig_pdb).heavy.copy()
    lig_ag._setCoords(lig_ag.getCoords(), overwrite=True)
    ...
    for step in range(nsteps):
        ...
        new_lig_coords = model(rec_G, lig_G)[0, 1]
...
def main():
    ...
    rec_pdb = case_dir / 'rec.pdb'
    lig_pdb = case_dir / 'lig_clus.pdb'
    crys_mol = case_dir / 'lig_orig.mol'
    refine(rec_pdb, lig_pdb, crys_mol, crys_mol=crys_mol)
