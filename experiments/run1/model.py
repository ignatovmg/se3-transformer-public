...
class SE3Refine(nn.Module):
    def __init__(self, rec_feature_size, rec_edge_dim, lig_feature_size, lig_edge_dim, emb_size, num_layers):
        super().__init__()
        self.rec = SE3Transformer(num_layers, [(rec_feature_size, 0), (1, 1)], rec_feature_size, [(emb_size, 0)],
                                  num_degrees=4, edge_dim=rec_edge_dim, div=4, pooling=None, n_heads=1, num_fc=0)
        self.lig = SE3Transformer(num_layers, [(lig_feature_size, 0)], lig_feature_size, [(emb_size, 0)],
                                  num_degrees=4, edge_dim=lig_edge_dim, div=4, pooling=None, n_heads=1, num_fc=0)
        self.cross = SE3Transformer(num_layers, [(emb_size, 0)], emb_size, [(1, 1)],
